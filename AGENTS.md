...
**Toggle: `GEMINI_USE_DONATED_OPENROUTER_KEY`** (default `true` since 2026-06-16). Only affects OpenRouter Gemini routing; does NOT touch grounded search. When on, OpenRouter Gemini calls prefer the donated `OAI_ANTH_OPENROUTER_KEY` wrapper with personal-key fallback; set false-y (`false`/`0`/`no`) to force `OPENROUTER_API_KEY`-only routing. On by default because Metaculus raised the Google rate limits and the donated key now serves most Gemini (e.g. `gemini-3.5-flash`, `gemini-3.1-flash-lite`). **Exception:** models in `DONATED_KEY_BLOCKED_GOOGLE_MODELS` (currently `gemini-3.1-pro`, prefix-matched — covers `gemini-3.1-pro-preview`, our forecaster slot) are *pinned* to the personal key even when the toggle is on: the donated account routes Google via a free-tier BYOK key with no Pro free tier (quota 0), so a donated attempt always 429s and falls over anyway. The pin skips that wasted round-trip + the CI-red fallback-counter bump; remove the entry once Metaculus fixes the BYOK routing (see the `TODO(gemini-3.1-pro-donated)` tag). See `metaculus_bot/fallback_openrouter.py:should_route_via_donated_key`.
